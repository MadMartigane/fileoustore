...
export const getPermission = (
	...
	return getPermissionStmt.get(datasetId, userId) as Permission | undefined;
};

export const getPermissionsByDatasetId = (
	datasetId: string,
): Pick<Permission, "userId" | "canRead" | "canWrite">[] => {
	return getPermissionsStmt.all(datasetId) as Pick<
		Permission,
...
export const userExists = (userId: string): boolean => {
	return !!checkUserExistsStmt.get(userId);
};

export const datasetExists = (datasetId: string): boolean => {
	return !!checkDatasetExistsStmt.get(datasetId);
};
